...
graph LR
    Client["Client (User)"]
    HAProxy["HAProxy (Reverse Proxy)<br/>Handles SSL termination, caching,<br/>and load balancing"]
    LB["Load Balancer<br/>Distributes traffic to multiple<br/>API Gateway instances"]
    Traefik1["Traefik Instance 1<br/>Manages authentication, rate limiting,<br/>and API routing"]
    Traefik2["Traefik Instance 2<br/>Redundant API Gateway instance<br/>for high availability"]
    Service1["User Service<br/>Handles user-related operations"]
    Service2["Order Service<br/>Processes orders and transactions"]
    Service3["Inventory Service<br/>Manages inventory data"]

    Client -->|HTTPS Request| HAProxy
    HAProxy -->|Load Balancing| LB
    LB -->|Forward API request| Traefik1
    LB -->|Forward API request| Traefik2

    Traefik1 -->|Route /users| Service1
    Traefik1 -->|Route /orders| Service2
    Traefik1 -->|Route /inventory| Service3

    Traefik2 -->|Route /users| Service1
    Traefik2 -->|Route /orders| Service2
    Traefik2 -->|Route /inventory| Service3
```
